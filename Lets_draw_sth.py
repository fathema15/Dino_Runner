...
# Stars for night sky (2D overlay positions)
...
def spawn_obstacle():
    ...
    x = 28.0
    z = 0.0
    if kind=="cactus": w,h,d=0.9,1.6,0.7; y=GROUND_Y
    else: w,h,d=1.0,0.7,0.9; y=1.7  # air for crouch
...
def spawn_cloud():
    x = 28.0; z = random.choice([-2,2]) * random.uniform(12.0, 16.0) ; y = random.uniform(3.8,6.5)
...
def update(dt):
    ...
    if invuln_t > 0: invuln_t = max(0, invuln_t - dt)
    if dino["jumping"]:
        dino["vy"] += GRAVITY*dt
        dino["y"] += dino["vy"]*dt
        if dino["y"] <= GROUND_Y:
    ...
    speed_mul = 1.0 + min(game_time * 0.008, 0.3)
    ...
    for c in coins[:]:
        ...
        if aabb(dino["x"],dino["y"]+dh/2,dino["z"],dino["w"],dh,dino["d"],
            ...
            score += 10
            coins.remove(c)
        if c["x"] < -40: coins.remove(c)
    for cl in clouds[:]:
        cl["x"] -= cl["speed"]*speed_mul*dt
        if cl["x"] < -80: clouds.remove(cl)
    speed_factor = (0.985)**(score//5)
    SPAWN_T -= dt; COIN_T -= dt; CLOUD_T -= dt
    if SPAWN_T<=0: SPAWN_T = max(0.3, random.uniform(SPAWN_INT_BASE, SPAWN_INT_BASE+SPAWN_INT_JIT)*speed_factor); spawn_obstacle()
    if COIN_T<=0: COIN_T = max(0.2, random.uniform(0.8, 1.4)*speed_factor); spawn_coin()
    if CLOUD_T<=0: CLOUD_T = max(0.25, random.uniform(CLOUD_INT_BASE, CLOUD_INT_BASE+CLOUD_INT_JIT)); spawn_cloud()
    if lives <=0: game_over=True; running=False
...
def display():
    ...
    for o in obstacles:
        if o["type"]=="cactus": draw_cactus(o)
        else: draw_ptero(o)
    for c in coins: draw_coin(c)  # eggs
    draw_dino()
    glDisable(GL_LIGHTING)
    begin_2d()
...
def keyboard(key, x, y):
    ...
    if key == 'p': running = not running
    if key == 'c': view_first_person = not view_first_person
    if key == 'n': is_night = not is_night
    if key == 'r':
    ...
    if key == 'q' and game_over: sys.exit()
...
def special_keyboard(key, x, y):
    ...
    if key == GLUT_KEY_LEFT:  cam_yaw -= 5.0
    if key == GLUT_KEY_RIGHT: cam_yaw += 5.0
    if key == GLUT_KEY_UP:    cam_dist -= 1.0
    if key == GLUT_KEY_DOWN:  cam_dist += 1.0
    if key == GLUT_KEY_PAGE_UP:   cam_pitch += 3.5
    if key == GLUT_KEY_PAGE_DOWN: cam_pitch -= 3.5
